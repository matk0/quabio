...
@router.get("/rag/variants")
async def get_variants():
    """
    Get available RAG variants.
    """
    try:
        variants = RAGServiceFactory.get_all_variants()
        return {
            "variants": [
                {
                    "id": variant.value,
                    "name": display_name,
                    "description": f"RAG using {display_name.lower()}"
                }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Chyba pri získavaní variantov: {str(e)}"
        )
...
async def debug_variants():
    """
    Debug endpoint to check both RAG variants.
    """
    try:
        results = {}
        
        # Test both variants
        for variant in [RAGVariant.FIXED_SIZE, RAGVariant.SEMANTIC]:
            try:
                chain = get_rag_chain_for_variant(variant)
                vs_stats = chain.vector_store.get_stats()
                
                # Test retrieval
                test_docs = chain.vector_store.similarity_search("chlad", k=3)
                ...
                results[variant.value] = {
                    ...
                    "test_retrieval": {
                        ...
                        "sample_titles": [doc.metadata.get('title', 'No title') for doc in test_docs[:3]]
                    }
                }
            except Exception as e:
                results[variant.value] = {
